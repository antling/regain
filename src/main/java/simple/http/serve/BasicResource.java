/*
 * BasicResource.java February 2001
 *
...
public abstract class BasicResource implements Resource {
   ...
   /**
    * Constructor that creates a <code>BasicResource</code> without
   ...
   /**
    * Constructor for the <code>BasicResource</code> is given the
    * <code>Context</code> so that it can generate status reports.
    * Every implementation of the <code>BasicResource</code> needs
    ...
    */
   protected BasicResource(Context context){
    ...
    */
   private void handle(Request req, Response resp, Report report){
      try{
         if(!resp.isCommitted()) {
            resp.reset();
    ...
    * @exception Exception this can throw an error for anything
    */
   public void process(Request req, Response resp, Report report) throws Exception{
    ...
    * of the <code>BasicResource</code>. This is used so that if there
